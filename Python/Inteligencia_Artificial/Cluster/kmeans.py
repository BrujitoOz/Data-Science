...
def KMeansClustering(n, kmeans, y_kmeans, titulo, xlabel, ylabel):
    ...
    for i in range(n):
        plt.scatter(x[y_kmeans == i, 0], x[y_kmeans == i, 1], s = 100, c = color[i], label = "Cluester {}".format(i+1))
    ...
    plt.legend()
    plt.show()
KMeansClustering(5, kmeans, y_kmeans, "Cluster de clientes", "Ingresos anuales (en miles de $)","Puntuacion de gasos (1-100)")