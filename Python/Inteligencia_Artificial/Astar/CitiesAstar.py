#%%
# Grafo no direccional - bidireccionl
# Arista: la primera ciudad es la instancia Edge en sí
# y la ciudad B a la que se conecta
class Edge(object):
...
#%%
# Nodo: la ciudad que le corresponde, h = heuristica
# dado el nombre city, se sabra el valor numerico de la heuristica definida abajo
# fs = h + g(distancia)
...
#%%
# Lista de nodos para poder evaluar
# para buscar una ciudad dada
class NodeList(list):
    def find(self, city):
    ...
    for v in graph[cities[n.city]]:
        ...
        if m:
            ...
            if m:
                ...
                m.parent_node = n
                openList.append(m)
                print("f(n)= ", m.fs, " g(n)= ", n_gs+dist, " + h(n)= ", m.hs, " ciudad=", v.cityB)
n = endNode
sol = []
while True:
    sol.append(n.city)
